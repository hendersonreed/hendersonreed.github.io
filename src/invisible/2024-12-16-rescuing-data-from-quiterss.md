...
That's too much of a mess to read. Let's try to prune it down to just the essentials. Someone in a Github issue for exporting data from QuiteRSS.sh recommended `select title,xmlUrl from feeds`. `.describe feeds` showed me that I could add `htmlUrl` to the output also.
...
But as I said above, ask and ye shall receive:
...
It gave me some nice examples of the basic format, which I won't post here, because I'm not interested in actually writing anything that outputs it. I'm interested in getting ChatGPT to write it *for me.*

After all, sometimes I just want the software - NOT the experience of writing it. In this case, I want the output (having my RSS feeds in newsboat), NOT the joy of writing an SQLite-to-OPML utility. Other times, I want the experience of writing it from scratch - I wouldn't use an LLM for Advent of Code, for example.
...
I think Maggie Appleton lays out a really interesting potential future in [her talk on home-cooked software](https://maggieappleton.com/home-cooked-software), and it's certainly influenced how I see these technologies - often empowering and maybe enriching, not parasitic or purely hype.
